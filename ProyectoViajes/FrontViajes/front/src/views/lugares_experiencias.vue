    ...
    >
      ...
      <ul class="list-group list-group-flush">
        <li class="list-group-item">
          Localización: {{ lugares.localizacion }}
        </li>
        <li class="list-group-item">País: {{ lugares.pais }}</li>
        <li class="list-group-item">
          Valoración de nuestra gente: {{ lugares.valoracion_media }}
        </li>
        <li class="list-group-item">
          Enclaves de Interes: {{ lugares.enclaves_de_interes }}
        </li>
        <li class="list-group-item">Fecha: {{ lugares.fecha }}</li>
      </ul>
    </div>
    <div class="rutaAdd">
      <router-link to="/addViajes"
        >¿Eres Usuario? Recomienda tu viaje!</router-link
      >
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    mostrarLugares() {
      var self = this;
      axios
        .get("http://localhost:3051/lugares_experiencias")
        // SI TODOO OK
        .then(function(response) {
          console.log(response);
          self.lugares_experiencias = response.data;
        })
        // SI SALE MAL
